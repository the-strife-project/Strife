...
void install() {
	...
	ISO9660_entity* stage1_e = ISO9660_get(stage1_p);
	if(!stage1_e) {
		printf("[FAILED]\n"
	...
	ISO9660_entity* stage2_e = ISO9660_get(stage2_p);
	if(!stage2_e) {
		printf("[FAILED]\n"
	...
	jotafs.newfile(stage2_e->length, stage2, 0, JOTAFS::FILETYPE::REGULAR_FILE, 0);
	jfree(stage2_e);
	printf("[OK]\n");
	...
	*/
	...
	jotafs.newfile(kernel_e->length, kernel, 0, JOTAFS::FILETYPE::REGULAR_FILE, 0);
	jfree(kernel);
	jfree(kernel_e);
	printf("[OK]\n");
	...
	ISO9660_entity* mss_e = ISO9660_get(mss_p);
	if(!mss_e) {
		printf("[FAILED]\n"
	...
	uint32_t mss_inoden = jotafs.newfile(mss_e->length, mss, 0, JOTAFS::FILETYPE::REGULAR_FILE, 0);
