...
uint8_t JRAMFS_model::getLevel(uint32_t i) {
...
uint32_t JRAMFS_model::discardLowerLevels(uint32_t i, uint8_t level) {
...
uint32_t JRAMFS_model::getSequentialBlock(const INODE& inode, uint32_t i) {
	// Fast comparison.
...
void JRAMFS_model::putBlockInInode(INODE& inode, uint32_t i, uint32_t block) {
	uint8_t level = getLevel(i);
	if(level == 0) {
		inode.DBPs[i] = block;
		return;
