...
uint32_t JOTAFS::getrecursive(uint8_t level, uint32_t i, uint32_t recLBA) {
	...
	uint32_t toRet;
	if(level > 1) toRet = getrecursive(level-1, i, next_recLBA);
	else toRet = next_recLBA;
	last_maxlevel = 0;
	return toRet;
}

uint32_t JOTAFS::gimmetheblocc(struct JOTAFS_INODE* inode, uint32_t i) {
	if(i > 9+(128*128*128)) {
		// Quadruply indirect block pointer.
		return getrecursive(4, i, inode->ext_4);
	} else if(i > 9+(128*128)) {
		// Triply indirect block pointer.
		return getrecursive(3, i, inode->ext_3);
	} else if(i > 9+128) {
		// Doubly indirect block pointer.
		return getrecursive(2, i, inode->ext_2);
	} else if(i > 9) {
		// Singly indirect block pointer.
		return getrecursive(1, i, inode->ext_1);
	} else {
		// Direct.
		return inode->DBPs[i];
	}
...
void JOTAFS::readwholefile(uint32_t LBAinode, uint8_t* buffer) {
	...
	for(uint32_t i=0; i<size_in_blocks; i++) {
		// Get the block no. i
		uint32_t thisblock = gimmetheblocc(inode, i);

		// Read it.
		uint8_t* thisblock_contents = iface.read28(thisblock);
...
uint8_t* JOTAFS::allocate_and_readwholefile(uint32_t LBAinode) {
	struct JOTAFS_INODE* inode = (struct JOTAFS_INODE*)(iface.read28(LBAinode));
	uint64_t size = inode->size;
	jfree(inode);

	uint8_t* buffer = (uint8_t*)jmalloc(size);
	readwholefile(LBAinode, buffer);
	return buffer;
}
